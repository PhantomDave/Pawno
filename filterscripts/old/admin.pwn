...
{
        print("\n--------------------------------------");
		print(" [FILTERSCRIPT adminspec]");
        print(" Admin Spec - CARICATO");
        print("--------------------------------------\n");
}
...
public OnPlayerCommandText(playerid, cmdtext[])
{
	...
	if(pRank[playerid] < 3) return 0;

	cmd = strtok(cmdtext, idx);

	// SPECTATE A PLAYER
 	if(strcmp(cmd, "/reconplayer", true) == 0) {
	    new tmp[256];
		tmp = strtok(cmdtext, idx);

		if(!strlen(tmp)) {
			SendClientMessage(playerid, COLOR_WHITE, "[Uso:] /specplayer [playerid]");
			return 1;
		}
		specplayerid = strval(tmp);
		
		if(!IsPlayerConnected(specplayerid)) {
		    SendClientMessage(playerid, COLOR_RED, "[Errore] Questo player non è attivo.");
			return 1;
		}
	...
	// SPECTATE A VEHICLE
 	if(strcmp(cmd, "/reconveicolo", true) == 0) {
	    new tmp[256];
		tmp = strtok(cmdtext, idx);

		if(!strlen(tmp)) {
			SendClientMessage(playerid, COLOR_WHITE, "[Uso:] /specveicolo [idveicolo]");
			return 1;
		}
